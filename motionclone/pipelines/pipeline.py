...
class MotionClonePipeline(DiffusionPipeline):
    ...
    def single_step_video(self, latents_group, step_index, step_t, example_latents, text_embeddings, 
        ...
        # [uncondition_latent, example_latent, control_latent]
        # TODO: remove uncondition_app, condition_app
        if step_index < self.input_config.guidance_step:
            ...
            if self.input_config.use_spatial_module:
                temp_attn_value_example = self.get_spatial_attn_value(index_select=[0, 1])
            else:
                temp_attn_value_example = self.get_temp_attn_value(index_select=[0, 1], component='v') if 'v' in a_c else None
                temp_attn_key_example = self.get_temp_attn_value(index_select=[0, 1], component='k') if 'k' in a_c else None
                temp_attn_query_example = self.get_temp_attn_value(index_select=[0, 1], component='q') if 'q' in a_c else None
            ...
            if self.input_config.use_motionclone:
                ...
                print(f"loss ({a_c})  : {loss_value.item():.4f}")
